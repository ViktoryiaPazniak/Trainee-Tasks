...
df_people = spark.read.option('header', 'true').schema(people_schema).csv('D:/DA/Innowise/spark_demo_course-main/3_Spark_Basics/data/top_100_richest.csv')

df_country = spark.read.option('header', 'true').schema(country_schema).csv('D:/DA/Innowise/spark_demo_course-main/3_Spark_Basics/data/wiki_number_of_billionaires.csv')


df_people_filtered = df_people.withColumn('net_worth', regexp_extract(col('net_worth'), r'^\$(\\d+).*$', 1).cast('int')).filter(col('net_worth') > 60)
...
df_parquet = spark.read.schema(people_schema).parquet('D:/DA/Innowise/spark_demo_course-main/3_Spark_Basics/data_parquet')

df_parquet_filtered = df_parquet.filter(col('nationality') == 'Russia')

df_last = df_last.union(df_parquet_filtered)

df_last.write.option("header", 'true').mode('overwrite').csv("D:/DA/Innowise/spark_demo_course-main/3_Spark_Basics/1.csv")
...
with open('D:/DA/Innowise/spark_demo_course-main/3_Spark_Basics/plans.txt', 'w') as file:
    file.write(plans)
    
#я оставил оба варианта вывода планов, но второй который plans_1 предпочтительнее. Планы одинаковые

with open('D:/DA/Innowise/spark_demo_course-main/3_Spark_Basics/plans1.txt', 'w') as file:
    file.write(plans_1)
